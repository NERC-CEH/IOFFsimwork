create_prediction_stack <- function(data_stack, resolution, biasfield, dat1, mesh, spde, structured_only=F){
  ...
  if(length(data_stack$data$names$y) > 1){
    ...
    stack.pred.response <- inla.stack(data=list(y=ys),
                                      effects = list(list(data.frame(interceptA=rep(1,np))), list(uns_field=1:spde$n.spde)),
                                      A=list(1,A.pred),
                                      tag='pred.response')
  } else if("Ntrials" %in% data_stack$data$names) {
    stack.pred.response <- inla.stack(data=list(y=NA, Ntrials = rep(1,np)),
                                      effects = list(list(data.frame(interceptA=rep(1,np))), list(str_field=1:spde$n.spde)),
                                      A=list(1,A.pred),
                                      tag='pred.response')
  } else {
    stack.pred.response <- inla.stack(data=list(y=NA),
                                      effects = list(list(data.frame(interceptB=rep(1,np))), list(uns_field=1:spde$n.spde)),
                                      A=list(1,A.pred),
                                      tag='pred.response')   
