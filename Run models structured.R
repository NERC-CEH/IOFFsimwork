...
structured_model <- function(structured_data, 
                             dat1, 
                             biasfield, 
                             plotting=FALSE,
...
join.stack <- create_prediction_stack(stk_structured, 
                                      resolution = resolution, 
                                      biasfield = biasfield, dat1 = dat1, mesh, spde)
...
image.plot(1:max_x,1:max_y,
           ...
           ylab='',
           ylim = c(0, dim[[2]]),
           main="structured-mean of r.f",
           asp=1)#,
           #zlim=c(-4,1))

image.plot(list(x=dat1$Lam$xcol, 
                y=dat1$Lam$yrow, 
                z=t(dat1$rf.s)), 
           main='Truth', 
           asp=1)#,
...
xsd1 <- inla.mesh.project(proj1.struct.binom, 
