...
unstructured_model_sf <- function(unstructured_data, dat1, biasfield, dim = dim, plotting=FALSE){
  ...
  stk_unstructured_data <- inla.stack(data=list(y=y.pp, e = e.pp),
                                      effects=list(list(data.frame(interceptB=rep(1,nv+n)), env = c(covariate, unstructured_data$env)), list(Bnodes=1:spde$n.spde, biasfield = 1:spde$n.spde)),
                                      A=list(1,A.pp),
  ...
  formulaN = y ~  -1 + interceptB + env + f(Bnodes, model = spde) + f(biasfield, model = spde)
  ...
  xmean2 <- inla.mesh.project(proj1, result$summary.random$biasfield$mean)
