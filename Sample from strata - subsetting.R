...
sampleStrata <- function(strata, 
  ...
  if("Stratified" %in% type){
    ...
    for(r in c(1,cols)){
      # vertical strips at extreme ends
      endstrip <- seq(from = r, to = (cols*(rows-1)+r), by = cols)
      res_list[[count]] <- endstrip
      count = count + 1
    }
    
    # collect mid-left and mid-right vertical strips
    for(j in (cols-2)){
      for(r in c(1,(cols-(j-1)))){
        # vertical strips
    ...
    # horizontal strips at extreme ends
    for(r in c(1,rows)){
      # horizontal strips
      res_list[[count]] <- (cols*(r-1)+1):(cols*r)  
      count = count + 1
    }
    
    # mid-bottom and mid-top horizontal strips
    for(j in (rows-2)){
      for(r in c(1,(rows-(j-1)))){
        # horizontal strips
    ...
    names(samp_list) <- c("left", "right", "midleft", "midright", "bottom", "top", "midbottom", "midtop", "whole")
