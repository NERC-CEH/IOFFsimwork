# FUNCTION FOR VALIDATION - multiple runs

validation_function_multiple <- function(result, resolution, join.stack, model_type = c("unstructured", "structured", "joint"), unstructured_data=NULL,
  ...
  #differences <- (transformation(m.prd)-mean(transformation(m.prd)))-truth_grid # calculate differences
  ...
  if(summary_results == T){
    ...
    if(length(which(row.names(coefficients) %in% "interceptA")==T)>0){coefficients[row.names(coefficients) %in% "interceptA",] <- 
      coefficients[row.names(coefficients) %in% "interceptA",]}
    if(length(which(row.names(coefficients) %in% "interceptB")==T)>0){coefficients[row.names(coefficients) %in% "interceptB",] <- 
      coefficients[row.names(coefficients) %in% "interceptB",]}
    if(length(which(row.names(coefficients) %in% "interceptB")==T)==0){coefficients[row.names(coefficients) %in% "env",] <- 
      coefficients[row.names(coefficients) %in% "env",]}else{coefficients[row.names(coefficients) %in% "env",] <- 
        coefficients[row.names(coefficients) %in% "env",]}
    ...
    names(summary_results) <- c("Proto-table", "coefficients", "mean", "truth", "Worst_grid_cells", "Best_grid_cells")
